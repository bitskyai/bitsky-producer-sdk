 ...
 */
async function getIntelligencesAPI(baseURL, globalId, securityKey, context) {
  let logger = console;
  ...
  return new Promise((resolve) => {
    ...
    try {
      ...
      let url = joinURL(constants.DIA_METADATA_PATH, baseURL);
      ...
      http(
        ...
        .then((res) => {
          logger.debug(
            `[getIntelligences][Finish] intelligences number: ${_.get(
              res,
              "data.length"
            )}`
          );
        ...
        .catch((err) => {
          logger.error(`[getIntelligences][Fail]. Error: ${err.message}`, {
            error: err,
          });
...
async function updateIntelligencesAPI(
  baseURL,
  securityKey,
  intelligences,
  context
