...
const { sendIntelligencesToRetailer } = require("./apis/retailers");
...
class Producer {
  constructor(context, configs) {
    ...
    this.__type = constants.SERVICE_RETAILER_TYPE; // default type is "service producer"
   ...
   */
  async sendToRetailerAndDIA(intelligences) {
    const logger = _.get(this, "context.logger") || console;
    // make sure send intelligences to correct Retailer, in case, it contains multiple Retailers, so first category them
    logger.debug("[sendToRetailerAndDIA][Start]");
    let retailers = {};
    const configs = this.getConfigs();
    // Separate Retailer based on url and method, so it can send to correct Retailer
    // The reason is because it maybe contains multiple Retailer's intelligences
    for (let i = 0; i < intelligences.length; i++) {
      let baseUrl = _.get(intelligences[i], "retailer.baseURL");
      let method = _.get(intelligences[i], "retailer.callback.method");
      let callbackPath = _.get(intelligences[i], "retailer.callback.path");
      // any of those intelligences don't exist, then skip this item
      if (!baseUrl || !method || !callbackPath) {
        logger.debug(
          "sendToRetailerAndDIA->invalid intelligences, miss baseUrl, method or callbackPath. Skip this item.",
          intelligences[i]
        );
        continue;
      }
      ...
      if (!retailers[key]) {
        retailers[key] = {
          retailer: intelligences[i].retailer,
          intelligences: [],
        };
      }
      retailers[key].intelligences.push(intelligences[i]);
    }
        ...
        promises.push(
          new Promise(async (resolve) => {
            try {
              ...
              try {
                await sendIntelligencesToRetailer(
                  baseURL,
                  method,
                  callbackPath,
                  headers,
                ...
                logger.debug(
                  `[sendIntelligencesToRetailer][Fail]. Key: ${key}. Error: ${err.message}`,
                  { error: err }
                );
                let intelligences = _.get(retailers[key], "intelligences");
                // if send to Retailer fail, then change intelligences state to `FAILED`
              ...
              try {
                await updateIntelligencesAPI(
                  configs.BITSKY_BASE_URL,
                  configs.BITSKY_SECURITY_KEY,
                  _.get(retailers[key], "intelligences"),
                  this.context
                );
              } catch (err) {
              ...
              logger.error(
                `[sendToRetailerAndDIA][Fail]. Key: ${key}. Error: ${err.message}`,
                { error: err }
              );
  ...
  async endCollectIntelligencesJob() {
    ...
    try {
      ...
      try {
        await this.sendToRetailerAndDIA(this.__runningJob.totalIntelligences);
      } catch (err) {
        logger.error(
          `[endCollectIntelligencesJob->sendToRetailerAndDIA] shouldn't fail, something really bad happened! error: ${err.message}`,
          { jobId: _.get(this.__runningJob, "jobId"), error: err }
        );
      }
