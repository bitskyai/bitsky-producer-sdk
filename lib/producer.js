...
class Producer {
   ...
   * @param {array} tasks
   */
  async sendToRetailerAndDIA(tasks) {
    const logger = _.get(this, "context.logger") || console;
    ...
    for (let i = 0; i < tasks.length; i++) {
      ...
      if (!baseUrl || !method || !callbackPath) {
        logger.debug(
          "sendToRetailerAndDIA->invalid tasks, miss baseUrl, method or callbackPath. Skip this item.",
          tasks[i]
        );
        continue;
      }
  ...
  async endCollectTasksJob() {
    ...
    try {
      ...
      try {
        await this.sendToRetailerAndDIA(this.__runningJob.totalTasks);
      } catch (err) {
        logger.error(
          `[endCollectTasksJob->sendToRetailerAndDIA] shouldn't fail, something really bad happened! error: ${err.message}`,
          { jobId: _.get(this.__runningJob, "jobId"), error: err }
        );
      }
