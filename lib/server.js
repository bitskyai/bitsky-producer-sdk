...
const DEFAULT_CONFIGS = {
  ...
  PRODUCER_HOME: constants.PRODUCER_HOME,
...
class BaseService {
  constructor(configs) {
    ...
    if (!this.context.producer) {
      this.context.producer = new Producer(this.context, configs);
    }
  }
  ...
  getConfigs() {
    ...
    let configs = {
      ...
      PRODUCER_HOME: process.env.PRODUCER_HOME,
      LOG_LEVEL: process.env.LOG_LEVEL,
    };

    let producerConfigs = {};
    if (this.context.producer) {
      producerConfigs = this.context.producer.getConfigs();
    }
    ...
    // Make sure log files are stored relative to `PRODUCER_HOME`
    configs.LOG_FILES_PATH = path.join(configs.PRODUCER_HOME, configs.LOG_FILES_PATH);
  ...
  setConfigs(configs) {
    if (configs instanceof Object) {
      ...
      if (this.context.producer) {
        this.context.producer.setConfigs(configs);
      }
    }
  }

  type(type) {
    return this.context.producer.type(type);
  }

  worker(worker) {
    return this.context.producer.worker(worker);
  }
   ...
   *  {
        type,
        githubURL: "https://github.com/bitskyai/bitsky-service-producer", // or "https://github.com/bitskyai/bitsky-headless-producer", depend on your producer type
        ...
        items: [
          {
            url: "/producer",
            title: "producer",
            description: "Currently Producer Configuration",
          },
          {
  ...
  routers(indexOptions) {
    try {
      ...
      this.app.use("/producer", producerRouter(this.context));
  ...
  /**
   * Destory this producer
   */
  async stop() {
    try {
      // stop producer
      if (this.context.producer) {
        await this.context.producer.stop();
      }
      ...
      this.app = undefined;
      if(this.context){
        this.context.producer = undefined;
