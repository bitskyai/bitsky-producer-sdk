...
const { getAgentAPI } = require("./apis/producers");
...
class Agent {
  constructor(context, configs) {
    ...
    this.__agentError = null; // error happened when get producer
    // how many job ran
  ...
  async getAgentConfiguration() {
    ...
    try {
      // Get stored producer configuration information, normally need to get DIA Base URL and Agent Global Id
      logger.debug("getAgentConfiguration->configs: ", {
        configs,
      });
      // If Agent Global ID or DIA Base URL is empty, then return empty producer configuration
      if (!configs.BITSKY_BASE_URL || !configs.GLOBAL_ID) {
        logger.debug(
          `Agent GlobalId or BitSky BaseURL is empty, return Agent Config: ${configs}`
        );
        this.__agentError = {
          status: 400,
        ...
        logger.debug(
          `Get Agent Config from server. BitSky MetadData URL: ${configs.BITSKY_BASE_URL}, Agent Global ID: ${configs.GLOBAL_ID}, Security Key: ${configs.BITSKY_SECURITY_KEY}`
        );
        ...
        this.__agentError = null;
        agent = _.merge({}, constants.DEFAULT_AGENT_CONFIGURATION, agent);
        logger.debug("getAgentConfiguration->producer: ", { agent });
        return agent;
      }
    } catch (err) {
      ...
      if (err && err.status) {
        ...
        if (err.status === 404) {
          this.__agentError.message = `Cannot find any agent by ${configs.GLOBAL_ID}. Please check your GLOBAL_ID`;
        } else if (err.status === 401) {
          this.__agentError.message = `Please pass correct BITSKY_SECURITY_KEY. ${configs.BITSKY_SECURITY_KEY} is invalid`;
        } else if (err.status >= 500) {
          this.__agentError.message = `Internal server error`;
        } else if (err.status === 403) {
          this.__agentError.message = `The agent you want connect to was already connected by other agent instance. You need to disconnect it before you can connect or change another agent to connect`;
        } else if (err.status >= 400 && err.code === "00144000002") {
          this.__agentError.message = `Please set PRODUCER_SERIAL_ID, this is required`;
        } else if (err.status >= 400 && err.code === "00144000004") {
          this.__agentError.message = `The agent's type you are trying to connect isn't ${this.type()}, please change Agent Global ID which its type is ${this.type()}`;
        } else if (err.status >= 400) {
          this.__agentError.message = `Please check GLOBAL ID, PRODUCER_SERIAL_ID or BITSKY_SECURITY_KEY, some fileds isn't correct`;
        } else {
          this.__agentError.message = `Internal server error`;
        }
      } else {
  ...
  async stop() {
    ...
    try {
      ...
      logger.error(`Stop agent fail. Error: ${err.message}`, {
        error: err,
      });
    }
  }
