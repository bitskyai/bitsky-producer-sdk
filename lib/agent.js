...
class Agent {
  constructor(context, configs) {
    ...
    this.__agentError = null; // error happened when get agent
    // how many job ran
  ...
  getConfigs() {
    ...
    let configs = {
      ...
      AGENT_SERIAL_ID: process.env.AGENT_SERIAL_ID,
    ...
    if (!configs.AGENT_SERIAL_ID) {
      // if AGENT_SERIAL_ID doesn't exit then need to init
      try {
        ...
        if (!preferencesJSON.AGENT_SERIAL_ID) {
          preferencesJSON.AGENT_SERIAL_ID = uuid.v4();
          fs.writeJsonSync(preferencesPath, preferencesJSON);
        }
        configs.AGENT_SERIAL_ID = preferencesJSON.AGENT_SERIAL_ID;
      } catch (err) {
        const logger = _.get(this, "context.logger") || console;
        logger.error(`Init agent AGENT_SERIAL_ID fail. Error: ${err.message}`, {
          function: "getConfigs",
          error: err,
        });

        // create an in memorry AGENT_SERIAL_ID
        process.env.AGENT_SERIAL_ID = uuid.v4();
        configs.AGENT_SERIAL_ID = process.env.AGENT_SERIAL_ID;
      }
    }
  ...
  async getAgentConfiguration() {
    ...
    try {
      ...
      if (!configs.MUNEW_BASE_URL || !configs.GLOBAL_ID) {
        ...
        this.__agentError = {
          status: 400,
          message: "GLOBAL_ID and MUNEW_BASE_URL is required",
        };
        return undefined;
      } else {
      ...
      if (err && err.status) {
        ...
        if (err.status === 404) {
          ...
          this.__agentError.message = `Please pass correct MUNEW_SECURITY_KEY. ${configs.MUNEW_SECURITY_KEY} is invalid`;
          ...
          this.__agentError.message = `Please set AGENT_SERIAL_ID, this is required`;
          ...
          this.__agentError.message = `Please check GLOBAL ID, AGENT_SERIAL_ID or MUNEW_SECURITY_KEY, some fileds isn't correct`;
        } else {
          this.__agentError.message = `Internal server error`;
        }
      } else {
