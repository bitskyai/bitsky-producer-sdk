...
async function startCollectIntelligencesJob() {
  try {
    ...
    if (intelligences && !intelligences.length) {
      ...
      await __worker([], __runningJob.jobId, __currentAgentConfig);
      resetRunningJob();
      return true;
    }
    __ranJobNumber++;
    ...
    let promises = await __worker(intelligences, __runningJob.jobId, __currentAgentConfig);
