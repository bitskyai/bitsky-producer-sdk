  ...
  getConfigs() {
    ...
    if (!configs.PRODUCER_SERIAL_ID) {
      ...
      try {
        ...
        logger.error(`Init agent PRODUCER_SERIAL_ID fail. Error: ${err.message}`, {
          function: "getConfigs",
          error: err,
        });
  ...
  async getAgentConfiguration() {
    ...
    try {
      ...
      if (!configs.BITSKY_BASE_URL || !configs.GLOBAL_ID) {
        ...
        logger.debug(
          `Get Agent Config from server. BitSky MetadData URL: ${configs.BITSKY_BASE_URL}, Agent Global ID: ${configs.GLOBAL_ID}, Security Key: ${configs.MUNEW_SECURITY_KEY}`
        );
      ...
      if (err && err.status) {
        ...
        if (err.status === 404) {
          ...
          this.__agentError.message = `Please check GLOBAL ID, PRODUCER_SERIAL_ID or MUNEW_SECURITY_KEY, some fileds isn't correct`;
        } else {
          this.__agentError.message = `Internal server error`;
        }
      } else {
